...
export function InvestmentAnalytics() {
  ...
  const isLoading = isLoadingDecimals || isLoadingInvestorInfo || isLoadingFee || isLoadingPoolCriteria
  ...
  const investorStats = {
    totalDeposit: investorInfo && decimals ? Number(formatUnits(investorInfo.totalDeposit || 0n, decimals)) : 0,
    totalRewards: investorInfo && decimals ? Number(formatUnits(investorInfo.totalRewards || 0n, decimals)) : 0,
    lastDepositTime: investorInfo ? Number(investorInfo.lastDepositTime || 0n) : 0,
    lastClaimTime: investorInfo ? Number(investorInfo.lastClaimTime || 0n) : 0,
  }
  ...
  const rewardRate = poolCriteria ? Number(poolCriteria[1]) / 100 : 0
  const lockPeriod = poolCriteria ? Number(poolCriteria[2]) : 0
  const apy = rewardRate * (365 / lockPeriod)
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-center p-3 bg-purple-900/10 rounded-lg border border-purple-500/10">
                ...
                <div className="text-right">
                  <div className="font-medium text-purple-300">
                    {isLoading ? <Skeleton className="h-5 w-16 inline-block" /> : `${Number(feePercentage) / 100}%`}
                  </div>
                </div>
              </div>
            </div>
